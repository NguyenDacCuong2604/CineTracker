...
final class MovieDetailViewModel {
    ...
    private let getSavedMovie: GetSavedMovieUseCase
    private let markAsWatched: MarkAsWatchedUseCase
    private let watchlistPublisher: AnyPublisher<[SavedMovie], Never>
    ...
    init(
        ...
        getSavedMovie: GetSavedMovieUseCase,
        ...
        self.getSavedMovie = getSavedMovie
        self.markAsWatched = markAsWatched
        self.watchlistPublisher = watchlistPublisher

        observeWatchlistChanges()
    }

    private func observeWatchlistChanges() {
        watchlistCancellable = watchlistPublisher
    ...
    func refreshWatchlistStatus() {
        state.isInWatchlist = isInWatchlist.execute(movieID)
        state.savedMovie = getSavedMovie.execute(movieID)
    }

    func markWatched(rating: Double, review: String) async {
        do {
            try await markAsWatched.execute(.init(
                movieID: movieID,
                rating: rating,
                review: review
            ))
    ...
    func toggleWatchlist() async {
        guard case let .loaded(details) = state.detail,
